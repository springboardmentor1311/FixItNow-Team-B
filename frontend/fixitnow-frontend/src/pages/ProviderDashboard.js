...
const ProviderDashboard = () => {
  ...
  const requestSeenStorageKey = providerEmail
    ...
    : "";
  const reviewSeenStorageKey = providerEmail
    ? `fixitnow_seen_provider_reviews_${providerEmail}`
    : "";
  ...
  useEffect(() => {
    if (!providerEmail) {
      ...
      setLastSeenReviewsAt(0);
      setThreadSeenMap({});
      return;
    }

    setLastSeenRequestsAt(readSeenTimestamp(requestSeenStorageKey));
    setLastSeenReviewsAt(readSeenTimestamp(reviewSeenStorageKey));
    setThreadSeenMap(readThreadSeenMap(threadSeenStorageKey));
  }, [providerEmail, requestSeenStorageKey, reviewSeenStorageKey, threadSeenStorageKey]);
  ...
  useEffect(() => {
    if (!providerEmail) return;
    if (!location.pathname.startsWith("/provider/reviews")) return;

    const now = Date.now();
    setLastSeenReviewsAt(now);
    saveSeenTimestamp(reviewSeenStorageKey, now);
  }, [location.pathname, providerEmail, reviewSeenStorageKey]);
  ...
  const providerReviews = useMemo(
    () =>
      (portalData.reviews || [])
        .filter((review) => normalizeEmail(review.providerEmail) === providerEmail)
        .sort((a, b) => toEpochMs(b.createdAt) - toEpochMs(a.createdAt)),
    [portalData.reviews, providerEmail]
  );

  const providerUnreadReviews = useMemo(
    () =>
      providerReviews.filter((review) => toEpochMs(review.createdAt) > lastSeenReviewsAt),
    [providerReviews, lastSeenReviewsAt]
  );

  const providerReviewBadgeCount = providerUnreadReviews.length;
  ...
  const providerMenu = useMemo(
    () =>
      providerMenuBase.map((item) => {
        ...
        if (item.path === "/provider/reviews") {
          return { ...item, badgeCount: providerReviewBadgeCount };
        }
        return item;
      }),
    [providerRequestBadgeCount, providerMessageBadgeCount, providerReviewBadgeCount]
  );
  ...
  if (!providerEntry) {
    return (
      <PortalLayout portalTitle="Service Provider" menuItems={providerMenu}>
        ...
        <AiHelpWidget
          role="PROVIDER"
          userName={providerDraft.displayName || user?.name || "Provider"}
    ...
    <>
      ...
      <section className="portal-grid">
        ...
        <article className="portal-metric">
          ...
          <p className="portal-metric-note portal-ok">
      ...
      <section className="portal-card">
        <div className="portal-title-row">
          <h3>New Ratings & Reviews</h3>
          <p>{providerReviewBadgeCount} new</p>
        </div>
        <div className="portal-list">
          {providerUnreadReviews.length === 0 && (
            <article className="portal-list-item">
              <h5>No new review notifications</h5>
              <p>Customer ratings and comments will appear here.</p>
            </article>
          )}
          {providerUnreadReviews.slice(0, 5).map((review) => (
            <article className="portal-list-item" key={review.id}>
              <h5>
                {review.customerName || "Customer"} rated you {Number(review.rating || 0).toFixed(1)} / 5
              </h5>
              <p>
                {review.subcategory ? `Service: ${review.subcategory} | ` : ""}
                {review.createdAt ? new Date(review.createdAt).toLocaleString("en-IN") : ""}
              </p>
              <p>{review.text}</p>
            </article>
          ))}
  ...
  const providerRequestsView = (
    <section className="portal-card">
      <div className="portal-title-row">
        <h3>Incoming Service Requests</h3>
        <p>Accept, reject, and mark service completion</p>
      </div>
        ...
        {providerBookings.map((booking) => (
          <article className="portal-list-card" key={booking.id}>
            ...
            {booking.status === "Completed" && (
              <p className="portal-list-sub">
                Customer has been notified to submit rating and review.
              </p>
            )}
          </article>
        ))}
  ...
  const providerReviewsView = (
    <section className="portal-card">
      <div className="portal-title-row">
        ...
        <p>{providerReviews.length} total</p>
      </div>
      ...
      <section className="portal-grid">
        ...
        <article className="portal-metric">
          <p className="portal-metric-value">{providerReviews.length}</p>
          <p className="portal-metric-label">Total reviews</p>
        </article>
        <article className="portal-metric">
          <p className="portal-metric-value">{providerReviewBadgeCount}</p>
          <p className="portal-metric-label">New notifications</p>
        </article>
      </section>
      ...
      <div className="portal-button-row">
        <button
          ...
          onClick={() => {
            ...
            setLastSeenReviewsAt(now);
            saveSeenTimestamp(reviewSeenStorageKey, now);
            setFeedback("Review notifications marked as read.");
          }}
          disabled={providerReviewBadgeCount === 0}
        >
          Mark All Review Notifications Read
        </button>
      </div>
      ...
      <div className="portal-list" style={{ marginTop: 12 }}>
        {providerReviews.length === 0 && (
          <article className="portal-list-item">
            ...
            <p>Reviews from completed customer bookings will appear here.</p>
          </article>
        )}
        {providerReviews.map((review) => {
          const isUnread = toEpochMs(review.createdAt) > lastSeenReviewsAt;
          return (
            <article className="portal-list-item" key={review.id}>
              <h5>
                {review.customerName || "Customer"} | {Number(review.rating || 0).toFixed(1)} / 5
                {isUnread ? " | NEW" : ""}
              </h5>
              <p>
                {review.subcategory ? `Service: ${review.subcategory} | ` : ""}
  ...
  return (
    <PortalLayout portalTitle="Service Provider" menuItems={providerMenu}>
      ...
      <Routes>
        ...
        <Route path="dashboard" element={providerHomeView} />
        <Route path="requests" element={providerRequestsView} />
        ...
        />
        <Route path="earnings" element={providerEarningsView} />
        <Route path="messages" element={providerMessagesView} />
        <Route path="reviews" element={providerReviewsView} />
        <Route path="settings" element={providerSettingsView} />
        <Route path="*" element={<Navigate to="dashboard" replace />} />
      </Routes>

      <AiHelpWidget
        role="PROVIDER"
        userName={providerDraft.displayName || providerEntry?.name || user?.name || "Provider"}
        userEmail={providerEmail}
        screen={location.pathname}
      />
    </PortalLayout>

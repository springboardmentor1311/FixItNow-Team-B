...
const MapProviderSearch = ({ services = [], onViewDetails }) => {
    ...
    const ratingOptions = useMemo(() => ["All", "4", "3", "2"], []);
    ...
    const filteredServices = useMemo(() => {
        ...
        return services.filter((s) => {
            ...
            const ratingMatch = Number(s.providerRating || 0) >= minRating;
            const locMatch = !searchQuery.trim()
                ...
                String(s.providerName || "")
                    .toLowerCase()
                    .includes(searchQuery.trim().toLowerCase());
            return catMatch && locMatch && ratingMatch;
        });
    }, [services, selectedCategory, searchQuery, selectedRating]);
                    ...
                    >
                        {ratingOptions.map((rating) => (
                            <option key={rating} value={rating}>
                                {rating === "All" ? "All ratings" : `${rating}.0+ rating`}
                            </option>
                        ))}
                    </select>
                    <button

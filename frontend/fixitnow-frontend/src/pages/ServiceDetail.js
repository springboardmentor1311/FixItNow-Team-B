...
const ServiceDetail = ({
    ...
    const completedBookingsForService = useMemo(() => {
        if (!service) return [];
        return customerBookings.filter((booking) => {
            if (booking.status !== "Completed") return false;
            if (normalizeEmail(booking.providerEmail) !== normalizeEmail(service.providerEmail)) {
                return false;
            }
            if (booking.serviceId && String(booking.serviceId) === String(service.id)) {
                return true;
            }
            return booking.category === service.category && booking.subcategory === service.subcategory;
        });
    ...
    const reviewedBookingIdSet = useMemo(
        ...
        [providerReviews, customerEmail]
    );

    const hasLegacyReviewForService = useMemo(
        () =>
            providerReviews.some((review) => {
                if (normalizeEmail(review.customerEmail) !== customerEmail) return false;
                if (String(review.bookingId || "").trim()) return false;
                if (String(review.serviceId || "").trim()) {
                    return String(review.serviceId) === String(service?.id || "");
                }
                return (
                    String(review.subcategory || "").trim().toLowerCase() ===
                    String(service?.subcategory || "").trim().toLowerCase()
                );
            }),
        [providerReviews, customerEmail, service]
    );

    const reviewableCompletedBooking = useMemo(
        () =>
            completedBookingsForService.find(
                (booking) => !reviewedBookingIdSet.has(String(booking.id || ""))
            ) || null,
        [completedBookingsForService, reviewedBookingIdSet]
    );

    const hasCompletedBooking = completedBookingsForService.length > 0;
    const alreadyReviewed = hasLegacyReviewForService || (hasCompletedBooking && !reviewableCompletedBooking);
    ...
    const handleReviewSubmit = () => {
        if (alreadyReviewed) {
            setFeedback("You've already reviewed this completed booking.");
            return;
        }
        if (!reviewableCompletedBooking) {
            setFeedback("Review unlocks only after provider marks your booking as completed.");
            return;
        }
        if (!reviewRating) {
        ...
        if (onAddReview) {
            onAddReview({
                bookingId: reviewableCompletedBooking?.id || "",
